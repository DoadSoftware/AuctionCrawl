...
public class IndexController 
{
	...
	public String initialisePage(ModelMap model) throws JAXBException, IOException, ParseException 
	{
		...
		if(current_date == null || current_date.isEmpty()) {
			...
			if(new File(AuctionUtil.AUCTION_DIRECTORY + AuctionUtil.CONFIGURATIONS_DIRECTORY + AuctionUtil.CRAWL_OUTPUT_XML).exists()) {
				session_Configurations = (Configurations)JAXBContext.newInstance(Configurations.class).createUnmarshaller().unmarshal(
						new File(AuctionUtil.AUCTION_DIRECTORY + AuctionUtil.CONFIGURATIONS_DIRECTORY + AuctionUtil.CRAWL_OUTPUT_XML));
			} else {
...
//				print_writer.println("LAYER3*EVEREST*STAGE*DIRECTOR*Loop START;");
				...
				break;
			case "ISPL":
				session_selected_scenes.add(new Scene("D:/DOAD_In_House_Everest/Everest_Cricket/Everest_ISPL_Auction_2024/Scenes/Ticker.sum"
						,"1")); // Front layer
				session_selected_scenes.get(0).scene_load(print_writer, session_selected_broadcaster);
				break;	
			}
